...
if true then
 delay(0, function()
   ...
   l.Image = "http://www.catium.xyz/asset/?id=23573247"
...
function reportContentProvider(time, queueLength, blocking)
	pcall(function()
		game:HttpGet("http://www.catium.xyz/Analytics/ContentProvider.ashx?t=" .. time .. "&ql=" .. queueLength, blocking)
	end)
end
function reportCdn(blocking)
	pcall(function()
		...
		if successDelta > 0 or failureDelta > 0 then
			game:HttpGet("http://www.catium.xyz/Game/Cdn.ashx?source=client&success=" .. successDelta .. "&failure=" .. failureDelta, blocking)
		end
	end)
end
...
function reportDuration(category, result, duration, blocking,errorType)
	...
	pcall(function() game:HttpGet("http://www.catium.xyz/Game/JoinRate.ashx?st=0&i=0&p=-1&c=" .. category .. "&r=" .. result .. "&d=" .. (math.floor(duration*1000)) .. "&ip=localhost&errorType=" .. errorType .. "&platform=" .. platform, blocking) end)
end
-- arguments ---------------------------------------
...
local success, err = pcall(function()	
	...
	playerConnectSucces, player = pcall(function() return client:PlayerConnect(<?=$settings->uid?>, "<?=$settings->ip?>", <?=$settings->port?>, 0, threadSleepTime) end)
	if not playerConnectSucces then
		--Old player connection scheme
		player = game:GetService("Players"):CreateLocalPlayer(<?=$settings->uid?>)
		analytics("Created Player")
		client:Connect("<?=$settings->ip?>", <?=$settings->port?>, 0, threadSleepTime)
	else
		analytics("Created Player")
	end
	...
	pcall(function() player.Name = [========[<?=$settings->username?>]========] end)
...
pcall(function() game:SetVideoInfo('<?xml version="1.0"?><entry xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://gdata.youtube.com/schemas/2007"><media:group><media:title type="plain"><![CDATA[ROBLOX Place]]></media:title><media:description type="plain"><![CDATA[ For more games visit http://www.catium.xyz]]></media:description><media:category scheme="http://gdata.youtube.com/schemas/2007/categories.cat">Games</media:category><media:keywords>ROBLOX, video, free game, online virtual world</media:keywords></media:group></entry>') end)
